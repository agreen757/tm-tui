...
// Implement Dialog interface methods
...
func (m *mockDialog) Update(msg tea.Msg) (Dialog, tea.Cmd) {
	m.updateCalled = true
	return m, m.updateReturnCmd
}

func (m *mockDialog) HandleKey(msg tea.KeyMsg) (DialogResult, tea.Cmd) {
	m.handleKeyCalled = true
	m.lastKeyMsg = msg
	return m.handleKeyResult, m.handleKeyReturnCmd
}
...
func (m *mockDialog) SetRect(width, height, x, y int) {}

func (m *mockDialog) GetRect() (width, height, x, y int) {
	return 0, 0, 0, 0
}

func (m *mockDialog) Title() string {
	return "Mock"
}

func (m *mockDialog) Kind() DialogKind {
	return DialogKindModal
}

func (m *mockDialog) ZIndex() int {
	return 0
}

func (m *mockDialog) SetZIndex(z int) {}

func (m *mockDialog) IsFocused() bool {
	return true
}

func (m *mockDialog) SetFocused(focused bool) {}

func (m *mockDialog) IsCancellable() bool {
	return true
}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.expectResult && test.result == DialogResultCancel {
				...
				if manager.HasDialogs() {
					t.Errorf("Expected dialog to be removed on %s", test.name)
				}
			}
		})
	}
